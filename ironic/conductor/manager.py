...
class ConductorManager(base_manager.BaseConductorManager):
    ...
    @messaging.expected_exceptions(exception.NoFreeConductorWorker)
    def pxeauto(self, context, node_id, data):
        LOG.debug('RPC pxeauto called for node %s', node_id)

        # NOTE(dtantsur): we acquire a shared lock to begin with, drivers are
        # free to promote it to an exclusive one.
        with task_manager.acquire(context, node_id, shared=True,
                                  purpose='pxeauto') as task:
            task.spawn_after(self._spawn_worker, task.driver.deploy.pxeauto,
                             task, data)
